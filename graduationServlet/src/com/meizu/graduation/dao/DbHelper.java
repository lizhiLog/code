...
public class DbHelper {
	...
	public String doGetMyWritePost(long id){
		...
		try {
			...
			String sql = "select post._id as id,appuser.photo as photo,appuser.name as name,"
					...
					+ "and post.author= "+id+" order by time desc";
	...
	public String doGetMyCommentPost(long id){
		JSONObject json=new JSONObject();
		json.put("code", 62);
		try {
			Statement statement=conn.createStatement();
			String sql = "select post._id as id,appuser.photo as photo,appuser.name as name,"
					+ "post.content as content ,post.time as time "
					+ "from appuser,post,postcomment  where appuser._id=post.author "
					+ "and post._id=postcomment.post and postcomment.author= "+id
					+ " order by time desc";
		resultSet=statement.executeQuery(sql);
		JSONArray data=new JSONArray();
		resultSet.beforeFirst();
		...
		System.out.println("mycoment"+json);
		return json.toString().trim();
	}
	
	public String doDeleteResource(long id){
		JSONObject json = new JSONObject();
		json.put("code", 60);
		try {
			Statement statement=conn.createStatement();
			statement.execute("SET SQL_SAFE_UPDATES=0");			
			statement.execute("delete from resource where _id="+id);
			json.put("result", 0);
		} catch (SQLException e) {
			json.put("result", 1);
			e.printStackTrace();
	...
	public String doUploadFile(ResourceData resourceData) {
		...
		try {
			...
			String sql = "insert into resource (type,subject,title,name,detail,time) values ("
					...
					+ resourceData.name
					+ "','"
					+ resourceData.detail + "'," + resourceData.time + ")";
	...
	public String doGetSubjectResource(long id, int type) {
		...
		try {
			...
			String sql = "select resource._id as id,resource.title as title"
					...
					+ " where resource.subject=" + id + " and resource.type="
					+ type;
			resultSet = statement.executeQuery(sql);
			resultSet.beforeFirst();
			while (resultSet.next()) {
	...
	public String doGetSubjectComment(long id) {
		...
		try {
			...
			// 查询学生人数
			String sql0 = "select count(*) as studentcount from selectsubject where selectsubject.subject="
					+ id;
			resultSet = statement.executeQuery(sql0);
			resultSet.first();
			data1.put("studentCount", resultSet.getInt("studentcount"));
			// 查询评论数目
			String sql1 = "select  count(*) as subjectcount from subjectcomment where subjectcomment.subject="
					+ id;
	...
	public String doGetTeacherSubject(long id) {
		...
		try {
			...
			String sql = "select  subject._id as id,subject.photo as photo,subject.name as name,appuser.name as author, "
					...
					+ " from subject,appuser  where  subject.author=appuser._id and subject.author= "
					+ id;
			resultSet = statement.executeQuery(sql);
			resultSet.beforeFirst();
			while (resultSet.next()) {
	...
	public String doSelectSubject(long userId, long subject) {
		...
		try {
			Statement statement = conn.createStatement();
			String sql0 = "select *  from selectsubject where subject="
					+ subject + " and user=" + userId;
			resultSet = statement.executeQuery(sql0);
			resultSet.last();
			...
			if (count != 0) {
				...
				String sql = "insert into selectsubject(user,subject) values ("
						+ userId + "," + subject + ")";
				int result = statement.executeUpdate(sql);
	...
	public String doGetStudentSubject(long id) {
		...
		try {
			...
			String sql = "select  subject._id as id,subject.photo as photo,subject.name as name,appuser.name as author, "
					...
					+ "and subject.author=appuser._id and selectsubject.user="
					+ id;
			resultSet = statement.executeQuery(sql);
			resultSet.beforeFirst();
			while (resultSet.next()) {
	...
	public String doDeleteSelectSubject(long user, long subject) {
		...
		try {
			Statement statement = conn.createStatement();
			resultSet = statement
					.executeQuery("select _id from selectsubject where user="
							+ user + " and subject=" + subject);
			resultSet.first();
			long id = resultSet.getLong("_id");
			int result = statement
					.executeUpdate("delete from selectsubject where _id=" + id);
			if (result > 0) {
				json.put("result", 0);
			} else {
				json.put("result", 1);
	...
	public String doGetComment(long id) {
		...
		try {
			...
			String sql = "select  appuser.photo as photo,appuser.name as name,post.content as content,post.time as time "
					+ "from post,appuser where post._id= "
					+ id
					+ " and post.author=appuser._id";
			resultSet = statement.executeQuery(sql);
			resultSet.first();
			String image = resultSet.getString("photo");
			data1.put("photo", image == null ? "empty" : image);
			...
			// 查询评论数目
			String sql1 = "select  count(*) as count from postcomment where postcomment.post="
					+ id;
			resultSet = statement.executeQuery(sql1);
			resultSet.first();
			...
			String sql2 = "select postcomment._id as id,appuser.photo as photo,appuser.name as name,postcomment.content as content, "
					+ "postcomment.time as time from postcomment,appuser where postcomment.author=appuser._id and postcomment.post="
					+ id;
			resultSet = statement.executeQuery(sql2);
			resultSet.beforeFirst();
			while (resultSet.next()) {
	...
	public String doModifyPassword(String email, String oldPassword,
			String newPassword) {
		JSONObject json = new JSONObject();
		try {
			Statement statement = conn.createStatement();
			resultSet = statement
					.executeQuery("select _id as id from appuser where email='"
			...
			if (count > 0) {
				...
				int result = statement
						.executeUpdate("update appuser set password='"
								+ newPassword + "' where _id=" + id);
				if (result > 0) {
					json.put("result", 0);
				} else {
					json.put("result", 2);
	...
	public String doLogin(UserData userData) {
		...
		try {
			...
			if (count != 0) {
				...
				String photo=resultSet.getString("photo");
				data.put("photo", photo==null?"empty":photo);
	...
	public String doUploadUserData(UserData userData){
		JSONObject json = new JSONObject();
		json.put("code", 52);
		try {
			Statement statement = conn.createStatement();
			System.out.println("xx");
			String sql = "update appuser set name='"+userData.name+"', photo='"+userData.photo
					+"', sex="+userData.sex+",school='"+userData.school+"', academy='"+userData.academy
					+"', detail='"+userData.detail+"'  where _id="+userData.id;
			int result = statement.executeUpdate(sql);
			System.out.println("xx");
			if (result > 0) {
				json.put("result", 0);
	...
	public String doDeletePost(long id){
		...
		try {
			Statement statement=conn.createStatement();
			statement.execute("SET SQL_SAFE_UPDATES=0");
			statement.execute("delete from post where _id="+id);
			statement.execute("delete from postcomment where post="+id);
			json.put("result", 0);
		} catch (SQLException e) {
			json.put("result", 1);
			e.printStackTrace();
	...
	public String doDeleteSubject(long id){
		JSONObject json = new JSONObject();
		json.put("code", 56);
		try {
			Statement statement=conn.createStatement();
			statement.execute("SET SQL_SAFE_UPDATES=0");
			statement.execute("delete from subject where _id="+id);
			statement.execute("delete from selectsubject where subject="+id);
			statement.execute("delete from subjectcomment where subject="+id);
			statement.execute("delete from resource where subject="+id);
			json.put("result", 0);
		} catch (SQLException e) {
			json.put("result", 1);
			e.printStackTrace();
	...
	public String doGetUserInfo(long id) {
		JSONObject json = new JSONObject();
		json.put("code", 54);
		try {
			Statement statement = conn.createStatement();
			resultSet = statement
					.executeQuery("select * from appuser where _id=" + id);
			if (resultSet.first()) {
				JSONObject value = new JSONObject();
				String photo = resultSet.getString("photo");
				System.out.println("photo--->" + resultSet.getString("photo"));
				...
				value.put("name", resultSet.getString("name"));
				value.put("sex", resultSet.getInt("sex"));
				value.put("school", resultSet.getString("school"));
				value.put("academy", resultSet.getString("academy"));
				String detail = resultSet.getString("detail");
				value.put("detail", detail == null ? "empty" : detail);
				json.put("result", 0);
				json.put("data", value);
			} else {
				json.put("result", 1);
	...
	public String doSendPost(PostData postData) {
		...
		try {
			...
			String sql = "insert into post (author,content,time) values ('"
					+ postData.author + "','" + postData.content + "',"
					+ postData.time + " )";
			int result = statement.executeUpdate(sql);
			if (result > 0) {
				jsonObject.put("result", 0);
	...
	public String doRegister(UserData userData) {
		...
		try {
			Statement statement = conn.createStatement();
			String sql0 = "select *  from appuser where email='"
					+ userData.email + "'";
			resultSet = statement.executeQuery(sql0);
			resultSet.last();
			int count = resultSet.getRow();
			if (count != 0) {
				...
				String sql = "insert into appuser(type,email,name,no,password,school,academy,time) values ('"
						...
						+ userData.school
						+ "','"
						+ userData.academy + "'," + userData.time + ")";
				int result = statement.executeUpdate(sql);
				if (result > 0) {
					jsonObject.put("result", 0);
				} else {
